...
class MLRetrievalCompletionFn(CompletionFn):
    """
    ...
    def __call__(self, prompt: Union[str, list[dict]], **kwargs: Any) -> RetrievalCompletionResult:
        ...
        self.embeddings_df["distance"] = _distances_from_embeddings(embedded_prompt, self.embeddings_df["embedding"].values)


        # Compute the cosine similarity between the prompt and the embeddings
        topk = " ".join(
            self.embeddings_df.sort_values('distance', ascending=True).head(self.k)["text"].values
        )
